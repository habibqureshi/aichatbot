#!/usr/bin/env python3
"""
Interactive Twilio media-stream simulator for local testing — **order voice** routes.

Same behavior as `twilio_media_stream_mimic.py`, but documented for:
  - POST `/api/v1/order/ws/greeting`  → TwiML with `<Stream url=...>` pointing at order OpenAI stream
  - WebSocket `/api/v1/order/openai/stream`  → `order_service.openai_stream` (mu-law @ 8kHz)

Note: The order router does not define `/api/v1/order/ws/stream-call` (that path exists on
appointment for `stream_call`). For order, use `openai/stream` when passing `--ws-url`.

Behavior:
1) Optionally calls your order webhook (`order_ws_greeting`) as Twilio would.
...
Usage:
  python twilio_media_stream_mimic_order.py \\
    --webhook-url http://127.0.0.1:8001/api/v1/order/ws/greeting

  python twilio_media_stream_mimic_order.py \\
    --ws-url 'ws://127.0.0.1:8001/api/v1/order/openai/stream?CallSid=CAxxx&From=%2B15551230000&To=%2B15557654321'

(Adjust host/port if your uvicorn bind differs; only `docs` may live under `/ai` in this app.)
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Twilio Media Stream simulator (order voice: /api/v1/order/ws/greeting "
            "and /api/v1/order/openai/stream)"
        ),
    )
    parser.add_argument(
        "--webhook-url",
        help=(
            "Order ws_greeting URL (TwiML with <Stream url=...>), e.g. "
            "http://127.0.0.1:8001/api/v1/order/ws/greeting"
        ),
    )
    parser.add_argument(
        "--ws-url",
        help=(
            "Direct WebSocket URL (skip webhook), e.g. "
            "ws://127.0.0.1:8001/api/v1/order/openai/stream?CallSid=...&From=...&To=..."
        ),
    )
    parser.add_argument(
        "--call-sid",
